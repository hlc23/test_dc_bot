...
class Basic(Cog_basic):
    ...
    async def me(self, ctx:discord.ApplicationContext):
        ...
        joinTime = f"{ctx.author.joined_at.date()} {((ctx.author.joined_at.time().hour)+8):0>2}:{ctx.author.joined_at.time().minute:0>2}:{ctx.author.joined_at.time().second:0>2}"
    ...
    async def show(
        ...
        joinTime = f"{self.member.joined_at.date()} {((self.member.joined_at.time().hour)+8):0>2}:{self.member.joined_at.time().minute:0>2}:{self.member.joined_at.time().second:0>2}"
        embed.add_field(name="加入時間", value=joinTime, inline=False)
